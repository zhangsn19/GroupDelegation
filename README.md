...
- **网络拓扑**：固定星形网络 — 被试居中心担任委托者，下游受托节点执行上报
- **主因变量**：上报指令相对真实骰子结果的偏离程度
- **流程**：知情同意 → 群聊介绍 → 前置问卷 → 任务规则 → 骰子委托 → 结果公布 → 后置问卷 → 事后说明

| Cell | 条件代码 | 受托类型 | 团队规模 | 度中心性 |
|------|----------|----------|----------|----------|
| 1 | `human_small` | 人类（陈磊） | 3 人 | 2 |
| 2 | `human_large` | 人类（陈磊） | 5 人 | 4 |
| 3 | `ai_small` | AI 助手 | 3 人 | 2 |
| 4 | `ai_large` | AI 助手 | 5 人 | 4 |

### 实验二：法律文书审查（角色 × AI 拓扑，3×3）

- **因子一**：被试层级位置（起草者 / 审核者 / 签发合伙人）
- **因子二**：AI 拓扑位置（下游起草 / 上游指令 / 旁路平行审核）
- **网络拓扑**：层级链「起草 → 审核 → 签发」，AI 嵌入不同位置改变责任流动方向
- **主因变量**：植入错误捕获率、核验行为次数、审查时长
- **流程**：角色代入 → 团队介绍 → 前置问卷 → 备忘录审查 → 后置问卷 → 事后说明

| 角色 \ AI 位置 | `downstream` | `upstream` | `bypass` |
|----------------|--------------|------------|----------|
| 起草者 `drafter` | `drafter_downstream` | `drafter_upstream` | `drafter_bypass` |
| 审核者 `reviewer` | `reviewer_downstream` | `reviewer_upstream` | `reviewer_bypass` |
| 合伙人 `partner` | `partner_downstream` | `partner_upstream` | `partner_bypass` |

## 拓扑与调试模式

### 强制条件（测试用）

在 URL 中传入 `experiment` 和 `condition` 可跳过随机分配：

```
# 实验一：AI 受托 + 大团队
http://localhost:3456/?experiment=experiment1&condition=ai_large

# 实验一：人类受托 + 小团队
http://localhost:3456/?experiment=experiment1&condition=human_small

# 实验二：审核者 + AI 旁路平行审核
http://localhost:3456/?experiment=experiment2&condition=reviewer_bypass
```

也可在 `admin.html` 页面查看全部 4 + 9 个条件的快捷链接。

### UI 拓扑呈现

- **实验一**：群聊侧栏显示星形网络图（您居中心 → 受托节点），成员列表标注「委托者」「受托节点」「组员」等关系标签
- **实验二**：侧栏显示层级链（起草 → 审核 → 签发）及 AI 位置（上游/下游/旁路），成员标注上下级关系
...
| 字段 | 说明 |
|------|------|
| `condition.cell` | 条件代码，如 `ai_large`、`reviewer_bypass` |
| `topology` | 拓扑描述与网络指标（度中心性、路径长度等） |
| `condition_agentType` / `condition_teamSize` | 实验一因子（CSV 导出） |
| `condition_role` / `condition_aiTopology` | 实验二因子（CSV 导出） |
| `topology_degreeCentrality` | 度中心性 |
| `topology_pathLengthToPartner` | 到签发节点路径长度（实验二） |
| `topology_betweennessCentrality` | 中介中心性 |

查询可用条件列表：`GET /api/conditions/experiment1` 或 `experiment2`。
...
| `condition` | 随机分配的条件（如 `agentType`, `teamSize` 或 `role`, `aiTopology`）；含 `cell` 代码 |
| `topology` | 网络拓扑指标与描述（度中心性、路径长度、summary 等） |
