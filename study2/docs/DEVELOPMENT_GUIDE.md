...
## .env 与临时 DATA_DIR
开发时可设置 `DEBUG_LINKS=true`、`REQUIRE_PARTICIPANT_ID=false`。临时验收建议设置 `DATA_DIR=%TEMP%\group-deception-study2-final-preview`。
...
## 常见错误排查
缺 participant ID：检查招募平台 URL 参数。admin 401：检查 `X-Admin-Token`。收入页 out of range：确认申报收入不高于实际收入且使用 cents。刷新丢进度：检查是否使用同一 participant_id 和 DATA_DIR。

## 禁止操作
不要运行 Git，不要部署真实服务，不要删除 data、node_modules、.gitkeep、.env.example，不要修改 GroupDelegation，不要擅自修改 PEER_INCOME_TEMPLATE 金额。
