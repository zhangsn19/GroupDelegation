...
async function main() {
  ...
  try {
    ...
    for (let index = 0; index < tokenEntries.length; index += 1) {
      ...
      const stimulusSnapshot = JSON.stringify({
        die_sequence: raw.study1_die_sequence,
        peer_display_order: raw.study1_peer_display_order,
        peer_records_by_round: raw.study1_peer_records_by_round
      });
      const conditionResume = await post(baseUrl, {
      ...
      assert.strictEqual(JSON.stringify({
        die_sequence: resumedRaw.study1_die_sequence,
        peer_display_order: resumedRaw.study1_peer_display_order,
        peer_records_by_round: resumedRaw.study1_peer_records_by_round
      }), stimulusSnapshot, `${descriptor.condition} stimuli changed during recovery`);
    }
    ...
    timingRaw.status = "task_in_progress";
    await fs.writeFile(path.join(dataDir, `${timingRaw.id}.json`), `${JSON.stringify(timingRaw, null, 2)}\n`);
    await postPath(baseUrl, `/api/session/${timingRaw.id}/dice/presented`);
    await postPath(baseUrl, `/api/session/${timingRaw.id}/decision-timing`, { round_index: 1, decision_segment_id: "segment_one", decision_segment_active_ms: 1200, decision_segment_hidden_ms: 100 });
    ...
    await postPath(baseUrl, `/api/session/${timingRaw.id}/decision-timing`, { round_index: 1, decision_segment_id: "segment_two", decision_segment_active_ms: 800, decision_segment_hidden_ms: 50 });
    await postPath(baseUrl, `/api/session/${timingRaw.id}/dice/round`, { round_index: 1, reported_value: 1, decision_segment_id: "segment_two", decision_segment_active_ms: 1000, decision_segment_hidden_ms: 80 });
    timingRaw = JSON.parse(await fs.readFile(path.join(dataDir, `${timingRaw.id}.json`), "utf8"));
    assert.strictEqual(timingRaw.dice_rounds[0].decision_time_ms, 2200);
    assert.strictEqual(timingRaw.dice_rounds[0].page_hidden_duration_ms, 180);
    assert.strictEqual(timingRaw.dice_rounds[0].timer_resumed_after_reload, true);
