...
function qualityFlags(session) {
  ...
  if (prolificScope && !session.completion_redirect_initiated_at) flags.push("completion_not_confirmed");
...
function participants(sessions, env) {
  return sessions.map((session) => {
    ...
    return {
      ...
      peer_identity: session.peer_identity || (normSupplement.usesHumanAiTaskPresentation(session.protocol_version) ? "" : "human_legacy"),
